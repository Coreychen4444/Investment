...
tags:
  ...
  - review
aliases:
  - STOCK_D Reactive Add Review
  - Ladder Discipline Corrections
---

# Review — STOCK_D Reactive Add & Ladder Discipline Corrections

## Summary
2026-06-05 美股 -4% 板块杀跌中,反射性加仓 STOCK_D Jan 2027 $60C × 2 @ $15.50(naked,IV 88%)。恰好落在预设 zone2,但属**运气位置,非纪律执行 → process D**。复盘从这笔逆推出 ladder 本质 + 四处修正,固化为「不确定性下的加仓执行系统」。

STOCK_D = 一只存储主题 ETF,作为某 HBM 龙头 re-rating 的代理(该龙头的 OTC ADR 几乎无成交,ETF 为唯一可执行 vehicle)。
...
- **宏观**: Nasdaq -4.18%、VIX +40% 到 21.5、强 NFP(+17.2万)+ STOCK_C 未上调 AI 指引 → 半导体板块连杀 = **macro/sector 驱动,非 STOCK_D 个股 fundamental 破裂**
...
## System produced
「**不确定性下的加仓执行系统**」: 买 sized 档 ≠ 赌底;**Q-A 资格 / Q-B·C 节奏 / Q-D 工具** 三旋钮分离(不矛盾);指标 = 证据非预言。详见 [[uncertainty-execution-system]],工具 `regime_score.py`。

### 如果 6/5 跑这套系统会输出什么
- **Q-A**: 标的 -15% + 同业 SMH -9.2/SOXX -10.4/STOCK_Y -13.3 + VIX +40% → **MACRO PANIC → GO**(thesis intact)
...
## Cross-refs
- [[uncertainty-execution-system]](执行层主文档)
- [[bayesian-decision-model]](决策层)
- [[trading-rules]](Ladder discipline 规则 10-12)
