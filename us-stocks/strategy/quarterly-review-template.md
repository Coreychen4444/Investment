...
1. **读取全部策略文件**：以 `README.md`（本目录索引）+ [[trading-rules]] 顶部 master index 为准——
   **不在本模板硬编码文件清单**（2026-06-11 修正：旧清单只覆盖 9/25 个文件，防漂移模板自己漂移最重）。
   逐文件检查：是否有与 master index 脱节的孤儿、内部矛盾阈值、过期 placeholder。

2. **读取本季度交易日志 + 跑量化报告**：
   - `python3 quant/report/perf_report.py` — realized/胜率/分层/classification/process×outcome 矩阵（统计不再手数）
   - Your trade journal (maintain separately) + `journals/options_journal.md`（叙事）
   - `journals/scores.jsonl`（结构化评分）+ `events/<YYYY-MM>/trades.jsonl`（fill 流）
