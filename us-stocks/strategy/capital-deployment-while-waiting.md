...
Applied when: 等某笔大 entry（LEAPS / 重仓 probe / catalyst 入场）时账户有闲置 cash，且当前 IV 在偏高区间 (percentile ≥ 60%，IV 闸门表见 [[options-strategy-framework]] §六)。
...
| IV 回归 + 标的暴跌（4/28 类砸盘） | 0% | spread max loss 但**触发 zone1 加仓机会 + IV crush 后 LEAPS 便宜** |

**4 个场景都比 hold cash 好**。这就是 sell premium 的非对称美。

## 执行约束
1. Spread 占用 margin ≤ 计划 LEAPS 预算（不超支等待）
2. Strike 必须过 strike-triangulation 检验（≥ 3 信号，见 [[strike-triangulation]]）
3. 单笔 sell put 评分 ≥ 7/8（见 [[sell-put-rules]]）且不触发抵消防守 veto
4. 受 concentration override protocol 约束（见 [[position-tiers]]）

## 源案例
**2026-04-30 STOCK_Y spread while waiting for LEAPS**：
- 等 IV 回归后开 STOCK_Y Jan 2027 $600/$800 spread（LEAPS 替代）
- 此期间 deploy 3× STOCK_Y 460/450 6/18 bull put spread 收 IV peak premium
- 同方向 thesis（押 IV 回归 + STOCK_Y 不破支撑）
- 4 场景 EV 都 > hold cash
