...
**先分退出类型，重进 gate 不同（2026-07-17）**：
- **A 结构性退出**（棘轮/趋势线/止损触发）→ 结构破了，重进 gate = 修复信号（🔓 机械盯，见下）
- **B 止盈退出且结构未破**（G-01 trim / trim 区 / target 到价）→ **无"修复"概念，随时可重进**：zone 先重锚（突破失效 → parabolic 锚），重进点 = 新 zone1 浅回踩/突破回测 + higher-low 确认（没确认 = 最多 probe size）；全平后重进 = **新 campaign**，size 不继承递减链，但集中度 cap 与 no-chase 照管。上次卖价与重进无关——"卖在 X 不肯 Y 买回" = hindsight anchor，才是真卖飞

- 结构修复定义：收盘突破触发离场的那个 lower high（= 新 HH）→ no-add lock 自动解除
- 修复后按**新 tranche** 走正常 entry 纪律重进（ladder / higher-low / zone 检查全套适用）
- **机械化（2026-07-17）**：监控脚本的**重进监控**（reentry watch） — 全平离场且结构破坏的标的持续盯收盘，close > 退出时高水位 → **🔓 一次性告警**（90 天窗口，必须覆盖 STOCK_Y 案例的 68 天洗出→修复周期）。**只提醒不硬拦**——严格 lock（拦重进直到修复）已被全样本回测否决（§8）。回测立项依据：STOCK_Y 被趋势线洗出后无重进信号 = 全部出场变体中最大单项反事实损失
...
3. 参数改动必须记录 §8 并说明理由，且 **2026-07-17 起必须附全样本反事实回测对比数字**（入场保持历史实际、退出按规则栈重放，三世界终值 + maxDD）——防"这次不一样"式软化。两个"直觉更稳"的改法（破位确认缓冲、严格重进门）都已被回测否决：**先跑数字再动参数**
...
| 2026-07-17 | 全栈回测基线 | 实际 = 峰值浮盈全数回吐至 ≈打平 → 规则全栈保住峰值 ~27% / 期权-only（正股价值持有豁免）保住 ~52%；maxDD 收窄约 2/3；同期 QQQ +18.2% | 全样本反事实首跑（6 个月 / 142 事件，入场不变退出重放）；STOCK_M/STOCK_D/STOCK_IN 为非母案例独立复现（合计 +5 位数）——参数非拟合的最强证据 |
| 2026-07-17 | 破位确认缓冲（<1N 首日不执行） | **否决** | 回测净负：真破位晚一日出场的代价 > 避掉的假破位；确认标定只留在结构位判断，不延伸到死亡线 |
| 2026-07-17 | 严格重进门（lock 存活到修复，拦重进） | **否决** | 回测净负：被拦的重进整体净正贡献；重进用 🔓 提醒，不硬拦 |
| 2026-07-17 | 重进监控过期窗 | 90d | 必须覆盖 STOCK_Y 案例 68d（洗出→修复） |
